...
def extract_unwatermarked_url(video_model_str: str, fallback_url: str) -> str:
    """Extracts unwatermarked video URL (base64/urlsafe) from video_model.video_list."""
    try:
        ...
        for v in video_list.values():
            ...
            decoded = decode_main_url(main_url)
            if decoded.startswith("http"):
        ...
        try:
            ...
            while time.time() - start < timeout:
                ...
                if videos:
                    url = videos[0]
                    if video_models:
                        url = extract_unwatermarked_url(video_models[0], fallback_url=url)
