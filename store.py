    ...
    def stats(self) -> dict:
        ...
        with _LOCK:
            ...
            all_time = self._conn.execute(
                "SELECT count(*), sum(status='completed'), sum(status='failed') FROM tasks"
            ).fetchone()

        all_tot = all_time[0] or 0
        all_comp = all_time[1] or 0
        all_fail = all_time[2] or 0
        today_comp = t["completed"]
        today_fail = t["failed"]
        today_tot = today_comp + today_fail

        return {
            "today_total": today_tot,
            "today_completed": today_comp,
            "today_failed": today_fail,
            "today_success_rate": round(today_comp / today_tot, 3) if today_tot else None,
            "all_time_total": all_tot,
            "all_time_completed": all_comp,
            "all_time_failed": all_fail,
            "all_time_success_rate": round(all_comp / all_tot, 3) if all_tot else None,
            "success_rate": round(all_comp / all_tot, 3) if all_tot else None,
            "total_tasks": all_tot,
            "completed_tasks": all_comp,
            "per_day": per_day,
            "trend": per_day,
